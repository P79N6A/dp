...
int init(int argc, char * argv [])
{
    ...
    do{
        ...
        /*step 4: 创建Net实例*/
        ProcessNet::get_mutable_instance().bindaddr(
        ...
        MyIndex::get_mutable_instance().set_input(new FileInput);
        rt=MyIndex::get_mutable_instance().update_index();
        if(rt != 0)
        {
            LOG_ERROR("ProcessNet query error[%d]\n", rt);
            break;
        }
        rt=MyIndex::get_mutable_instance().start_update_thread();
        if(rt != 0)
        {
            LOG_ERROR("ProcessNet query error[%d]\n", rt);
            break;;
