...
static _st_netfd_t *_st_netfd_new(int osfd, int nonblock, int is_socket)
{
  ...
  fd->inuse = 1;
  fd->next = NULL;
  fd->read_timeout = ST_UTIME_NO_TIMEOUT;
  fd->write_timeout = ST_UTIME_NO_TIMEOUT;
		       ...
		       st_utime_t timeout)
{
  ...
  while ((osfd = glibc_accept(fd->osfd, addr, (socklen_t *)addrlen)) < 0) {
    if (errno == EINTR)
      continue;
		       ...
		       st_utime_t timeout)
{
  ...
  for ( ; ; ) {
    if (p == NULL) {
      osfd = glibc_accept(fd->osfd, addr, (socklen_t *)addrlen);
    } else {
      /* Get the lock */
	       ...
	       st_utime_t timeout)
{
  ...
  while (glibc_connect(fd->osfd, addr, addrlen) < 0) {
    if (errno != EINTR) {
      /*
	...
	return -1;
      ...
      n = sizeof(int);
      if (glibc_getsockopt(fd->osfd, SOL_SOCKET, SO_ERROR, (char *)&err,
		     (socklen_t *)&n) < 0)
	return -1;
      if (err) {
	errno = err;
...
ssize_t st_read(_st_netfd_t *fd, void *buf, size_t nbyte, st_utime_t timeout)
{
  ...
  while ((n = glibc_read(fd->osfd, buf, nbyte)) < 0) {
    if (errno == EINTR)
      continue;
    if (!_IO_NOT_READY_ERROR)
      return -1;
		   ...
		   st_utime_t timeout)
{
  ...
  while (*iov_size > 0) {
    if (*iov_size == 1)
      n = glibc_read(fd->osfd, (*iov)->iov_base, (*iov)->iov_len);
    else
		    ...
		    st_utime_t timeout)
{
  ...
  while (*iov_size > 0) {
    if (*iov_size == 1)
      n = glibc_write(fd->osfd, (*iov)->iov_base, (*iov)->iov_len);
    else
		...
		int *fromlen, st_utime_t timeout)
{
  ...
  while ((n = glibc_recvfrom(fd->osfd, buf, len, 0, from, (socklen_t *)fromlen))
	 < 0) {
    if (errno == EINTR)
      continue;
    if (!_IO_NOT_READY_ERROR)
	      ...
	      const struct sockaddr *to, int tolen, st_utime_t timeout)
{
  ...
  while ((n = glibc_sendto(fd->osfd, msg, len, 0, to, tolen)) < 0) {
    if (errno == EINTR)
      continue;
    if (!_IO_NOT_READY_ERROR)
      return -1;
