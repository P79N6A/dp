...
using namespace poseidon::feedback;
            ...
            if(pid >0)
            {
                ...
                }else
                {
                    ...
                    if(Config::get_mutable_instance().ha_on())
                    {
                        ...
                        nrt=HA_REG("fb",Config::get_mutable_instance().local_ip(),Config::get_mutable_instance().server_port());
                        if(nrt != 0)
                        {
...
//        if(!poseidon::util::Func::single_instance(Config::get_mutable_instance().pid_file()))
//        {
...
//        }
        ...
        /*step 4: 创建Net实例*/
        FbServer::get_mutable_instance().bindaddr(
        ...
        CommFactoryInterface::instance().add_comm(&(FbServer::get_mutable_instance()));

#if 0
        rt=TagAccess::get_mutable_instance().init(Config::get_mutable_instance().redis_host(), Config::get_mutable_instance().redis_port());        
        if(rt != 0)
        {
            LOG_ERROR("TagAccess init error[%d]", rt);
            rt=-1;
            break;
        }
#endif
        rt=RedisPool::get_mutable_instance().init(Config::get_mutable_instance().redis_host(), Config::get_mutable_instance().redis_port());
        if(rt != 0)
        {
            LOG_ERROR("RedisPool init error[%d]", rt);
            rt=-1;
            break;
        }


        RedisPool::get_mutable_instance().set_handle(REDIS_ADKEY, FbServer::s_on_get_adgroup);
        RedisPool::get_mutable_instance().set_handle(REDIS_MEMCAMPAIGN, FbServer::s_on_get_campaign);
        RedisPool::get_mutable_instance().set_handle(REDIS_ADNUM, FbServer::s_on_get_count);
        RedisPool::get_mutable_instance().set_handle(REDIS_ADVERTISTER, FbServer::s_on_get_advertister);
        RedisPool::get_mutable_instance().set_handle(REDIS_CREATIVE, FbServer::s_on_get_creative);
        RedisPool::get_mutable_instance().set_handle(REDIS_DEAL_ID, FbServer::s_on_get_deal);
        RedisPool::get_mutable_instance().set_handle(REDIS_DEAL_CAMPAIGN, FbServer::s_on_get_deal_campaign);
        RedisPool::get_mutable_instance().set_handle(YESTERDAY_COST, FbServer::s_on_get_yesterdayCost);
