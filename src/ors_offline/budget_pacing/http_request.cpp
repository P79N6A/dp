...
{  
    ...
    if(m_iSocketFd != INVALID_SOCKET) {  
        ...
        if(SocketFdCheck(m_iSocketFd) > 0) {  
            string strResult = HttpDataTransmit(strHttpHead.c_str(), m_iSocketFd);  
            if(0 < strResult.size()) {  
...
//构建HTTP消息头  
string HttpRequest::HttpHeadCreate(const char* strMethod, const char* strUrl, const char* strData)  
{  
    string strHost = GetHostAddrFromUrl(strUrl);  
    string strParam = GetParamFromUrl(strUrl);  
    
	char strHttpHead[BUFSIZE];
	...
	strcat(strHttpHead, "Content-Type: close\r\n");
	...
	string str_ret(strHttpHead);
    return str_ret;   
}  
  
  
//发送HTTP请求并接受响应  
string HttpRequest::HttpDataTransmit(const char *strHttpHead, const int iSockFd)  
{  
	char buf[BUFSIZE];
...
string HttpRequest::GetHostAddrFromUrl(const char* strUrl)  
{     
    char url[URLSIZE] = {0};  
    strcpy(url, strUrl);  
...
{     
    ...
    int iLen = strlen(strAddr);  
    char strParam[URLSIZE];
    memset(strParam, 0, URLSIZE);  
    int iPos = -1;  
    for(int i=0; i<iLen+1; i++) {  
        if(strAddr[i] == '/') {  
            iPos = i;  
...
{  
    int iPort = -1;  
    string strHostAddr = GetHostAddrFromUrl(strUrl);   
    if(strHostAddr.size() == 0) {  
        return -1;  
    }  
