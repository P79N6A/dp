            ...
            if(pid >0)
            {
                ...
                }else
                {
                    ...
                    if(Config::get_mutable_instance().ha_on())
                    {
                        ...
                        nrt=HA_REG("dn",Config::get_mutable_instance().local_ip(),Config::get_mutable_instance().server_port());
                        if(nrt != 0)
                        {
...
int init(int argc, char * argv [])
{
	...
	if(!poseidon::util::Func::single_instance(FLAGS_pid))
        ...
        rt=DnServer::get_mutable_instance().init();
        if(rt != 0)
		{
			LOG_ERROR("CommFactoryInterface::instance().init() error, return [%d]", rt);
			rt=-2;
        ...
        /*step 4: 创建Net实例*/
        DnServer::get_mutable_instance().bindaddr(
    	...
    	LOG_INFO("init DNServer instance");
        CommFactoryInterface::instance().add_comm(&(DnServer::get_mutable_instance()));
...
//            break;
//        }
//        RedisPool::get_mutable_instance().set_handle(RA_USER, DnServer::s_on_get_Creative);
