...
DEFINE_int32(redis_reconnect_interval, 3, "redis断线重连间隔，单位:秒");
DEFINE_int32(redis_cluster_info_interval, 5, "redis获取cluster info间隔，单位:秒");
...
public:
    static RedisConnChangeInfo * new_instance(RedisClient * client,
            int client_conn_no) {
        RedisConnChangeInfo * rcci = new RedisConnChangeInfo(client,
                client_conn_no);
        return rcci;
    }
...
void RedisClient::init(struct event_base * base, string host, int port,
        bool is_cluster) {
    if (_is_init)
        return;
    _is_init = true;
...
void RedisClient::on_connect(int redis_conn_no, int status) {
    ...
    if (status != REDIS_OK) {
        LOG_ERROR("redis connect error,%s:%d[%s]", rci->host.c_str(), rci->port,
                rci->redis_async_context->errstr);
        re_connect(rci);
        return;
    }
...
void RedisClient::on_disconnect(int redis_conn_no, int status) {
    ...
    LOG_ERROR("redis disconnect and re_connect,%s:%d", rci->host.c_str(),
            rci->port);
    re_connect(rci);
}

void RedisClient::get_cluster_info(boost::shared_ptr<RedisConnInfo> rci) {
    LOG_DEBUG("get cluster info,redis node %d %s:%d", rci->no,
            rci->host.c_str(), rci->port);
    RedisOnReply *on_reply = RedisOnReply::new_instance(
            boost::bind(&RedisClient::on_connect_cluster, this, _1));
    redisAsyncCommand(rci->redis_async_context, redis_command_cb,
            (void *) on_reply, "cluster slots");
}

void RedisClient::get_cluster_info() {
    RedisConnIter conn_iter;
    for (conn_iter = _redis_conn_infos.begin();
            conn_iter != _redis_conn_infos.end(); conn_iter++) {
...
void RedisClient::set_cluster_info_event() {
    ...
    util::Closure *cluster_closure = util::NewCallback(this,
            &RedisClient::get_cluster_info);
    _cluster_info_event = new util::TimeoutEvent(_base,
            FLAGS_redis_cluster_info_interval, 0, timeout_event_cb,
            (void *) cluster_closure);
...
void RedisClient::on_connect_cluster(redisReply *reply) {
    ...
    if (reply->type != REDIS_REPLY_ARRAY) {
        LOG_ERROR("command[cluster slots] error,reply type error[%s]",
                reply->str);
        return;
    }
    ...
    for (iter = redis_conn_infos.begin(); iter != redis_conn_infos.end();
            iter++) {
        cluster_info_string << iter->second->host << "|" << iter->second->port
                << "|" << iter->second->slot_start << "|"
                << iter->second->slot_end << "|";
    }
    ...
    if (cluster_info_string.str().compare(_cluster_info_string) != 0) {
        ...
        if (redis_conn_infos.size() > 0) {
            LOG_DEBUG("reflesh cluster map");

            dead_conn_infos = _redis_conn_infos;
            _redis_conn_infos.clear();

            _cluster_info_string = cluster_info_string.str();
            for (iter = redis_conn_infos.begin();
                    iter != redis_conn_infos.end(); iter++) {
                boost::shared_ptr<RedisConnInfo> rci = iter->second;
                rci->no = _client_info_num++;
                _redis_conn_infos[rci->no] = rci;
                connect(rci);
                for (int slot_no = rci->slot_start; slot_no <= rci->slot_end;
                        slot_no++) {
...
redisAsyncContext * RedisClient::get_redis_context(const string &key) {
    ...
    while (_is_init) {
        if (!_is_cluster) {
            ...
            boost::unordered_map<int, int>::iterator slot_iter =
                    _redis_slot_map.find(slot_num);
...
void RedisClient::connect(boost::shared_ptr<RedisConnInfo> rci) {
    ...
    rci->redis_async_context->data = (void *) RedisConnChangeInfo::new_instance(
            this, rci->no);
    redisLibeventAttach(rci->redis_async_context, _base);
    redisAsyncSetConnectCallback(rci->redis_async_context, redis_connect_cb);
    redisAsyncSetDisconnectCallback(rci->redis_async_context,
            redis_disconnect_cb);
}
...
void RedisClient::re_connect(boost::shared_ptr<RedisConnInfo> rci) {
    ...
    util::Closure *interval_closure = util::NewCallback(this,
            &RedisClient::connect, rci);
    rci->reconnect_event = new util::TimeoutEvent(_base,
            FLAGS_redis_reconnect_interval, 0, timeout_event_cb,
            (void *) interval_closure);
